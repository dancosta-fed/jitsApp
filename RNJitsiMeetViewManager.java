...
public class RNJitsiMeetViewManager extends SimpleViewManager<RNJitsiMeetView> implements JitsiMeetViewListener {
  ...
  public RNJitsiMeetViewManager(ReactApplicationContext reactContext,
      IRNJitsiMeetViewReference jitsiMeetViewReference) {
  ...
  @ReactProp(name = "iniciar", defaultBoolean = false)
  public void setCall(RNJitsiMeetView view, Boolean calling) {

    RNJitsiMeetModule iniciar = new RNJitsiMeetModule(mReactContext, mJitsiMeetViewReference);

    String url = "https://meet.jit.si/saudeagora123";
    ReadableMap userInfo = new JavaOnlyMap();
    ReadableMap meetOptions = new JavaOnlyMap();
    ReadableMap meetFeatureFlags = new JavaOnlyMap();

    System.out.println("\n >===> CHAMADA INICIADA <===<");

    if (calling) {
      System.out.println("\n >===> Entrou na CONDICIONAL <===<");
      iniciar.chamada(url, userInfo, meetOptions, meetFeatureFlags);
    }
  ...
  public Map getExportedCustomBubblingEventTypeConstants() {
    return MapBuilder.builder()
        .put("conferenceJoined",
            MapBuilder.of("phasedRegistrationNames", MapBuilder.of("bubbled", "onConferenceJoined")))
        .put("conferenceTerminated",
            MapBuilder.of("phasedRegistrationNames", MapBuilder.of("bubbled", "onConferenceTerminated")))
        .put("conferenceWillJoin",
            MapBuilder.of("phasedRegistrationNames", MapBuilder.of("bubbled", "onConferenceWillJoin")))
        .build();
  }
}